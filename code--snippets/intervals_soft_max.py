...
def main():
    ...
    # try to get X workers in time
    for i in range(num_intervals):
        works = [overlaps[e, i] for e in range(num_employees)]

        epsilon = model.NewIntVar(0, num_employees, f'delta_i{i}')

        model.Add(sum(works) >= soft_average_amount - epsilon)
        model.Add(sum(works) <= soft_average_amount + epsilon)
